...
const SkillButton = ({ name, textcolor, logo: Logo, border }) => {
  return (
    <button className="center gap-2 border font-medium relative border-neutral-200/[0.4] bg-[#151515] px-6 py-2 rounded-full">
      <span>
        <Logo className={`text-2xl ${textcolor}`} />
      </span>

      <span className={` font-sans text-sm tracking-widest `}>{name}</span>
      <span
